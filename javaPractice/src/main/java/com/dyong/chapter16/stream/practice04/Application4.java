package com.dyong.chapter16.stream.practice04;

/* 영화 데이터 분석
 * 영화 데이터를 분석하는 프로그램을 작성하세요.
 * 영화는 `제목`, `장르`, `평점`, `상영시간(분)` 정보를 가지고 있습니다.
 * 주어진 영화 리스트에서 다음 작업을 수행하세요.
 * */
...
public class Application4 {
    ...
    public static void main(String[] args) {
        ...
        List<String> highRatedActionMovies = movies.stream()
                .filter(movie -> movie.getGenre().equals("액션") && movie.getRating() >= 8.0)
                .map(Movie::getTitle)
                .toList();
        ...
        Map<String, String> topRatedMoviesByGenre = movies.stream()
                .collect(Collectors.groupingBy(Movie::getGenre,
                        Collectors.collectingAndThen(
                                ...
                                optionalMovie -> optionalMovie.map(Movie::getTitle).orElse(null))
                ));
