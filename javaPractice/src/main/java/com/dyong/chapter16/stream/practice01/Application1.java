package com.dyong.chapter16.stream.practice01;
...
public class Application1 {
    public static void main(String[] args) {
        ...
        System.out.println("모든 직원의 이름 : " + allNames);


        // 2. IT 부서에서 급여가 5,000 이상인 직원의 이름을 급여 순으로 내림차순 정렬하여 반환
        List<String> highSalaryIT = employees.stream()
                .filter(employee -> employee.getSalary() >= 5000 && employee.getDepartment().equals("IT"))
                .sorted((employeeA, employeeB) -> employeeB.getSalary() - employeeA.getSalary())    // 내림차순
        //        .sorted(Comparator.comparingInt(Employee::getSalary)) // 오름차순
